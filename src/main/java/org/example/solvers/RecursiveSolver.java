...
public class RecursiveSolver implements SolverInterface {

    private static boolean[] solve(int[][] inputTable, int capacity, int n) {
        boolean[] result = new boolean[inputTable[0].length];

        if (capacity == 0 || n == -1) {
            return result;
        } else if (inputTable[0][n] > capacity) {
            result = solve(inputTable, capacity, n - 1);
        } else {
            boolean[] tmp = solve(inputTable, capacity, n - 1);
            boolean[] tmp2 = or(insert(result, true, n), solve(inputTable, capacity - inputTable[0][n], n - 1));
    ...
    public KnapsackCaseSolution solve(KnapsackCase knapsackCase) {
        ...
        final boolean[] primitiveSolution = solve(primitiveTable, knapsackCapacity, items.size() - 1);
